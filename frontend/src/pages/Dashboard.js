...
const TYPE_BG = {
  Apartment: '#E1F5EE', House: '#EAF3DE',
  Villa: '#FAEEDA', Land: '#d1fae5', Commercial: '#F1EFE8',
};
...
function Dashboard() {
  ...
  const handleAdd = async (e) => {
    ...
    // Store type and emoji inside item_name so we can retrieve it later
    const emoji = PROPERTY_TYPES.find(t => t.type === selectedType)?.emoji || '🏠';
    ...
    try {
      ...
      showToast(setError, err.response?.data?.error || 'Failed to add.');
    }
  };
  ...
  // Parse item_name back into parts
  // Format stored: "Property Name||Type||Emoji"
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#f5f7f5' }}>
      ...
      <div style={s.header}>
        <div>
          ...
          <div style={s.hsub}>Welcome, {email}</div>
      ...
      <div style={s.body}>
        ...
        <div style={s.card}>
          ...
          <form onSubmit={handleAdd}>
            ...
            <input
              style={s.input}
              placeholder="Image URL (optional — leave empty to use type icon)"
              value={imageUrl}
              onChange={e => setImageUrl(e.target.value)}
            />

            <button style={s.addBtn} type="submit">+ Save to favourites</button>
          </form>
        </div>

        {/* Favourites Grid */}
        <div style={s.card}>
          <div style={s.clabel}>My favourites ({favourites.length})</div>
          {favourites.length === 0 ? (
            <p style={s.empty}>No saved properties yet. Add one above!</p>
          ) : (
            <div style={s.grid}>
              {favourites.map(fav => {
                ...
                return (
                  <div key={fav.id} style={s.propCard}>
                    ...
                    {/* Real image if URL saved, emoji bg if not */}
                    {fav.image_url ? (
                      <img
                        src={fav.image_url}
                        alt={name}
                    ...
                    {/* Emoji fallback — shown when no image_url or image fails */}
                    <div style={{
                      ...s.propEmoji,
...
const s = {
  header: { background: '#1D9E75', padding: '18px 28px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  htitle: { fontSize: '18px', fontWeight: '500', color: 'white' },
  hsub: { fontSize: '12px', color: 'rgba(255,255,255,0.85)', marginTop: '3px' },
  badge: { display: 'inline-block', background: 'rgba(255,255,255,0.18)', color: 'white', fontSize: '11px', padding: '2px 10px', borderRadius: '20px', marginTop: '4px' },
  logoutBtn: { background: 'white', color: '#1D9E75', border: 'none', padding: '7px 16px', borderRadius: '8px', fontSize: '13px', fontWeight: '500', cursor: 'pointer' },
  ...
  input: { width: '100%', padding: '9px 12px', border: '1px solid #d1d5db', borderRadius: '8px', fontSize: '14px', marginBottom: '10px', boxSizing: 'border-box', outline: 'none' },
  typeRow: { display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '12px' },
  typeBtn: { padding: '6px 14px', borderRadius: '20px', border: '1px solid #d1d5db', background: 'white', color: '#6b7280', fontSize: '12px', cursor: 'pointer' },
  typeBtnActive: { background: '#EAF3DE', color: '#3B6D11', border: '1px solid #97C459' },
  addBtn: { width: '100%', padding: '10px', background: '#1D9E75', color: 'white', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '500', cursor: 'pointer' },
  ...
  propBody: { padding: '12px', borderTop: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  propName: { fontSize: '13px', fontWeight: '500', marginBottom: '2px' },
  propType: { fontSize: '11px', color: '#1D9E75' },
  removeBtn: { padding: '5px 12px', background: '#FCEBEB', color: '#A32D2D', border: 'none', borderRadius: '6px', fontSize: '12px', cursor: 'pointer', whiteSpace: 'nowrap' },
  empty: { textAlign: 'center', color: '#9ca3af', fontSize: '14px', padding: '30px 0' },
};
